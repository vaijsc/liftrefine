...
from model.sgm.util import (avg_pool_nd, conv_nd, linear,
                                              normalization,
                                              timestep_embedding, zero_module)

from model.sgm.attention import SpatialTransformer, VolumetricTransformer
    ...
    def forward(
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock):
                ...
                x = layer(x, context)
            elif isinstance(module, VolumetricTransformer):
                x = layer(x, context)
            else:
                x = layer(x)
        return x
...
def get_default_unet(cfg):
    input_dim = cfg.model.unet.volume_out_channels
    model_channels = cfg.unet_diffusion.base_channels 
    return UNetModel(in_channels=input_dim,
                        model_channels=model_channels,
                        out_channels=input_dim,
                        num_res_blocks=2,
                        attention_resolutions=[2, 4, 8],
                        dropout = 0.0,
                        channel_mult = (1, 2, 4, 4), # 16 8 4 2 # 32 16 8 4
                        conv_resample = True,
                        dims = 3,
                        num_classes = None,
