...
# trainer class
class TrainerUpsampleVSD(object):
    def __init__(
        self,
        reconstructor,
        teacher_model,
        ...
        student_params = [p for n, p in self.student_model.named_parameters() if p.requires_grad]
        student_names = [n for n, p in self.student_model.named_parameters() if p.requires_grad]
        ...
        self.student_opt = Adam(student_params, lr=train_student_lr, betas=adam_betas)
        student_scheduler = get_cosine_schedule_with_warmup(self.student_opt, warmup_period, \
                                                    train_num_steps, constant=optimization_cfg.constant_lr)
        ...
        if self.accelerator.is_main_process:
            ...
            num_params = count_parameters(self.student_model)
            print(f"Total number of trainable parameters - student: {num_params // 1e6}M")
        
        self.reconstructor_ema = EMA(self.reconstructor, beta=ema_decay, update_every=ema_update_every, include_online_model=False)
        self.student_ema = EMA(self.student_model, beta=ema_decay, update_every=ema_update_every, include_online_model=False)
        ...
        self.student_model, self.student_opt, self.student_scheduler = self.accelerator.prepare(
                                                                    self.student_model, self.student_opt,\
                                                                    student_scheduler)
    ...
    def load_reconstructor(self, path):
        ...
        teacher_model = EMA(self.teacher_model, include_online_model=False)
        student_model = EMA(self.student_model, include_online_model=False)
        
        reconstructor.load_state_dict(data["reconstructor_ema"], strict=True)
        teacher_model.load_state_dict(data["diffusion_ema"], strict=True)
        student_model.load_state_dict(data["diffusion_ema"], strict=True)

        reconstructor.copy_params_from_ema_to_model()
        teacher_model.copy_params_from_ema_to_model()
        student_model.copy_params_from_ema_to_model()
    ...
    def train(self):
        ...
        with tqdm(
            ...
            while self.step < self.train_num_steps:
                ...
                total_loss_dict                   = {}
                total_loss_dict["train/vsd_loss"] = 0.0
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        student_model = accelerator.unwrap_model(self.student_model)
                        vsd_loss = student_model.foward_vsd_loss(batch, self.teacher_model, cfg=self.classifier_free_guidance)

                        vsd_loss /= self.gradient_accumulate_every
                        total_loss_dict["train/vsd_loss"] += vsd_loss.item()

                    self.accelerator.backward(vsd_loss)

                accelerator.clip_grad_norm_(self.reconstructor.parameters(), 1.0)
                total_loss = total_loss_dict["train/vsd_loss"]
                ...
                with self.accelerator.autocast():
                    diffusion_loss = self.student_model(batch, batch_idx=0)
                    total_loss_dict["train/diffusion_loss"] += diffusion_loss.item()

                self.accelerator.backward(diffusion_loss)

                accelerator.clip_grad_norm_(self.student_model.parameters(), 1.0)
                accelerator.wait_for_everyone()
                ...
                if accelerator.is_main_process and self.step % self.logging_every == 0:
                    ...
                    total_loss_dict["lr/diffusion"] = self.student_scheduler.get_last_lr()[0]
