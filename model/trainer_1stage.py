...
class Trainer1Stage(object):
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        latent = misc["latent"].detach()
                        diffusion_loss, misc_diffusion = self.diffusion_model(latent)
                ...
                if accelerator.is_main_process:
                    if self.step % self.summary_every == 0:
                        with torch.inference_mode():
                            ...
                            _, misc_diffusion   = self.diffusion_ema(misc_recons["latent"])
                    ...
                    with torch.inference_mode():
                        ...
                        while num_samples < self.num_samples:
                            ...
                            with self.accelerator.autocast():
                                ...
                                sample_latent            = diffusion_model.ddim_sample(latent.shape)
    ...
    def logger_summary_diffusion(self, misc):
        ...
        with torch.inference_mode():
            ...
            sample_latent        = diffusion_model.ddim_sample(cleaned_latent.shape)
