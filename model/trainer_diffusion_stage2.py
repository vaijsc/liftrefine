...
# trainer class
class TrainerDiffusionStage2(object):
    def __init__(
        self,
        reconstructor,
        diffusion_model,
        ...
        diffusion_params = [p for n, p in self.diffusion_model.named_parameters() if p.requires_grad]
        self.diffusion_opt = Adam(diffusion_params, lr=train_lr, betas=adam_betas)
        diffusion_scheduler = get_cosine_schedule_with_warmup(self.diffusion_opt, warmup_period, \
                                                    train_num_steps, constant=optimization_cfg.constant_lr)
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        volume_features = self.reconstructor.model.encode_volume(input_images, input_cameras)
                        _, rendered_features, _ = self.reconstructor.model.render_volumes(volume_features, rendered_cameras)
                ...
                total_loss = total_loss_dict["train/diffusion_loss"] 
                    ...
                    if self.step % self.summary_every == 0:  
                        with torch.inference_mode():
                            with self.accelerator.autocast():
                                ...
                                log_dict = self.diffusion_ema.ema_model.log_images(batch, N=256, plot_denoise_rows=True)
