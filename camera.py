...
def get_cameras_from_data_dict(cfg, data, device):
    # slicing_idxs are the cameras that are kept in the batch
    if cfg.data.dataset_type == "co3d":
        example_cameras = PerspectiveCameras(
                    R = data["R"].reshape([-1, 3, 3]),
                    T = data["T"].reshape([-1, 3]),
                    focal_length=data["focal_lengths"].reshape([-1, 2]),
                    principal_point=data["principal_points"].reshape([-1, 2]),
                    device=device
        )
